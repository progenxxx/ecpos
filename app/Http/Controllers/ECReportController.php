...
public function syncInventoryVariance(Request $request)
{
    try {
        ...
        Log::info('Starting inventory variance sync using optimized script', [
            'sync_date' => $syncDate,
        ...
        // Execute the complete inventory sync script from script/inventory.txt
        // Set Store and Date variables
        DB::statement("SET @storeName = ?", [$storeName]);
        DB::statement("SET @syncDate = ?", [$syncDate]);
        ...
        // Calculate and Update Ending Inventory for all records (following script logic)
        ...
        // Calculate and Update Variance for all records (following script logic: item_count - ending)
        ...
        if (Schema::hasTable('sync_logs')) {
            DB::table('sync_logs')->insert([
                ...
                'store_name' => $storeName,
                'user_id' => Auth::id(),
                'affected_records' => $varianceResult,
        ...
        Log::info('Inventory variance sync completed successfully using optimized script', [
            'sync_date' => $syncDate,
            'store_name' => $storeName,
            'total_affected_rows' => $totalAffectedRows
        ]);

        // Step 4: Show updated records
        ...
        return response()->json([
            ...
            'data' => [
                'sync_date' => $syncDate,
                'store_name' => $storeName,
                'records_updated' => count($updatedRecords),
                'total_affected_rows' => $totalAffectedRows,
