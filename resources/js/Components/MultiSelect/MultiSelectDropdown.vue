      ...
      <div class="border-b border-gray-200">
        ...
        >
          <input
            ...
            :checked="selectedCount === (options?.length || 0)"
            :indeterminate="selectedCount > 0 && selectedCount < (options?.length || 0)"
          >
...
const displayValue = computed(() => {
  ...
  if (count === 0) return 'Select stores...';
  if (count === 1) return `1 store selected`;
...
const toggleAll = () => {
  ...
  if (props.modelValue.length === allOptions.length) {
    emit('update:modelValue', []);
  } else {
    emit('update:modelValue', allOptions.map(option => option.NAME));
  }
};
