...
const calculatePartyCakes = (items) => {
  return items.reduce((sum, item) => sum + (Number(item.SRP || 0)), 0);
};
...
const printPackingList = () => {
  ...
  stores.forEach(([storeName, storeItems]) => {
    ...
    if (storeName === props.getstore && props.sptrans && props.sptrans.length > 0) {
      specialOrderContent = `
        <tr>
            ...
            <div v-for="storeName in storeNames" :key="storeName" class="mb-8">
              ...
              <div class="max-w-4xl mx-auto p-4 bg-white shadow-lg text-xs">
                <table class="w-full border-collapse border border-gray-400">
                  ...
                  <!-- SPECIAL ORDER ONLY -->
                  <template v-if="storeName === getstore && props.sptrans && props.sptrans.length > 0">
                    <tr>
                  ...
                  <!-- PARTY CAKES ONLY -->
                  <!-- <template v-if="storeName === getstore && props.partycakes && props.partycakes.length > 0"> -->
                  <template v-if="storeName === getstore && partycakes && (Array.isArray(partycakes) ? partycakes.length > 0 : Object.keys(partycakes).length > 0)">
                    <tr>
                      <td colspan="6" class="border border-gray-400 p-1 font-bold">PARTY CAKES ORDERS</td>
                    </tr>
                    <tr>
                      <td colspan="1" class="border border-gray-400 p-1">PARTYCAKES OS NO</td>
                      <td colspan="4" class="border border-gray-400 p-1 text-center">DESIGN</td>
                      <td class="border border-gray-400 p-1 text-center">AMOUNT</td>
                    </tr>
                    <tr v-for="(spItem, index) in props.partycakes" :key="index">
                      <td colspan="1" class="border border-gray-400 p-1 text-center">{{ spItem.COSNO }}</td>
                      <td colspan="4" class="border border-gray-400 p-1 text-center">{{ spItem.BDAYCODENO }}</td>
                      <td class="border border-gray-400 p-1 text-center">{{ spItem.SRP }}</td>
                    </tr>
                    <tr>
                      <td colspan="5" class="border border-gray-400 p-1 text-right font-bold">TOTAL</td>
                      <td class="border border-gray-400 p-1 text-right font-bold">{{ formatCurrency(calculatePartyCakes(props.partycakes)) }}</td>
                    </tr>
                  </template>
                  ...
                  <!-- <tr>
                    <td colspan="1" class="border border-gray-400 p-1">ORANGE CRATES</td>
                    <td colspan="1" class="border border-gray-400 p-1">BLUE CRATES</td>
                    <td colspan="2" class="border border-gray-400 p-1">EMPANADA CRATES</td>
                    <td colspan="3" class="border border-gray-400 p-1">BOX</td>
                  </tr> -->
                  <tr>
