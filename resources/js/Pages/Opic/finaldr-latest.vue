...
const specialOrders = computed(() => {
  return picklist.value.filter(item => Number(item.SPECIALORDER || 0) > 0);
});

const specialOrderTotalCost = computed(() => {
  return specialOrders.value.reduce((total, item) => {
    return total + (Number(item.COST || 0) * Number(item.SPECIALORDER || 0));
  }, 0);
});
    ...
    <template v-slot:main>
      ...
      <div role="tablist" class="tabs tabs-lifted mt-10 p-5">
        ...
        <div role="tabpanel" class="tab-content bg-base-100 border-base-200 p-6 h-[85vh] overflow-y-auto">
            ...
            <template v-else>
              <div v-for="(storeItems, storeName) in groupedPicklist" :key="storeName">
                  ...
                  <table class="w-full border-collapse border border-gray-400">
                    ...
                    <tr>
                      ...
                      <td colspan="2" class="border border-gray-400 p-1">
                        ...
                        DELIVERY DATE: {{ storeItems[0].DELIVERYDATE }}
                      </td>
                    </tr>
                    <tr>
                      <td colspan="3" class="border border-gray-400 p-1">RECEIVED FROM:<br><span class="font-bold">HEADOFFICE</span></td>
                      <td colspan="3" class="border border-gray-400 p-1">DELIVERED TO:<br><span class="font-bold">{{ storeItems[0].STORENAME }}</span></td>
                    </tr>
                    ...
                    <!-- <tr v-for="item in picklist" :key="item.ITEMID">
                      ...
                      <td class="border border-gray-400 p-1 text-center">{{ item.ACTUAL }}</td>
                    ...
                    <tr v-for="item in storeItems" :key="item.ITEMID">
                      <td class="border border-gray-400 p-1">{{ item.ITEMNAME }}</td>
                      <td class="border border-gray-400 p-1 text-center">{{ item.CHECKINGCOUNT }}</td>
                      <td class="border border-gray-400 p-1 text-center">{{ item.ACTUAL }}</td>
                      <td class="border border-gray-400 p-1 text-center">{{ item.ACTUAL - item.CHECKINGCOUNT }}</td>
                    ...
                    <template v-if="hasSpecialOrder">
                      ...
                      <tr v-for="specialOrder in specialOrders" :key="specialOrder.ITEMID">
                        <td class="border border-gray-400 p-1">{{ specialOrder.ITEMNAME }}</td>
                        <td class="border border-gray-400 p-1 text-center">{{ specialOrder.SPECIALORDER }}</td>
