<script setup>
import Create from "@/Components/StockCounting/Create.vue";
import Update from "@/Components/StockCountingLine/Update.vue";
import Post from "@/Components/StockCountingLine/Post.vue";
...
const navigateToOrder = (journalid) => {
  ...
  window.location.href = `/StockCountingLine/${journalid}`;
};
...
const post = async (journalId) => {
  ...
  try {
    ...
    console.log('Sending POST request to /post-stockcounting');
    const postResponse = await fetch('/post-stockcounting', {
      method: 'POST',
      headers: {
    ...
    <Main active-tab="STOCK">
        ...
        <template v-slot:main>
            ...
            <TableContainer>
                ...
                <DataTable :data="stockcountingtables" :columns="columns" class="w-full relative display" :options="options" >
