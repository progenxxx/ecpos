                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    ...
                    >
                        <div
                            ...
                            :key="card.title"
                            class="transform transition-all duration-300 hover:-translate-y-2 hover:scale-105"
                        >
                            <div 
                                class="bg-white/80  rounded-3xl shadow-xl hover:shadow-2xl border border-blue-100/50 p-6 space-y-4 relative overflow-hidden group"
                            >
                                <!-- Gradient Background Overlay -->
                                <div 
                                    class="absolute inset-0 opacity-10 group-hover:opacity-20 transition-all duration-300"
                                    :style="{background: `linear-gradient(135deg, ${card.color}, transparent)`}"
                                ></div>
                                
                                <div class="flex justify-between items-center relative z-10">
                                    <div 
                                        :class="[card.bgColor, 'p-3 rounded-xl shadow-md transform transition-all group-hover:rotate-12']"
                                    >
                                        <component 
                                            :is="card.icon" 
                                            :class="[card.color, 'w-7 h-7']"
                                        />
                                    </div>
                                </div>
